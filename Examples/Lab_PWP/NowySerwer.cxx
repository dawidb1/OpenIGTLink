...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
	...
	if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
	{
		...
		igtl::PointMessage::Pointer responsePointMsg = igtl::PointMessage::New();
		...
		for (int i = 0; i < nElements; i++)
		{
			...
			igtl::PointElement::Pointer responsePointElement;
			responsePointElement = igtl::PointElement::New();
			responsePointElement->SetName(pointElement->GetName());
			responsePointElement->SetGroupName(pointElement->GetGroupName());
			responsePointElement->SetRGBA(rgba);
			responsePointElement->SetPosition(responsePos);
			responsePointElement->SetRadius(pointElement->GetRadius());
			responsePointElement->SetOwner(pointElement->GetOwner());

			responsePointMsg->AddPointElement(responsePointElement);
		}

		igtl::Sleep(5000);
		responsePointMsg->Pack();
		socket->Send(responsePointMsg->GetPackPointer(), responsePointMsg->GetPackSize());
	}
